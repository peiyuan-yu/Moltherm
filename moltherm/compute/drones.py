...
class MolThermDrone(AbstractDrone):
    """
    A rewritten QChemDrong to parse QChem calculations and insert an organized, searchable entry into the database.
    """
    ...
    def generate_doc(self, dir_name, qcinput_files, qcoutput_files, multirun):
        try:
            ...
            for c in d["calcs_reversed"]:
                # Logic: Look at all possible jobs without having to
                # assume how jobs will look
                # Only replace values if they do not already exist
                if c["input"]["rem"]["job_type"] == "opt" or c["input"]["rem"]["job_type"] == "optimization":
                    molecule = c["molecule_from_optimized_geometry"]
                    energy = c["final_energy"]
                    d["output"]["optimized_molecule"] = d["output"].get("optimized_molecule",
                                                                        molecule)
                    d["output"]["final_energy"] = d["output"].get("final_energy",
                                                                  energy)
                    if c["opt_constraint"]:
                        opt_constraint = [c["opt_constraint"][0],
                                          float(c["opt_constraint"][6])]
                        d["output"]["constraint"] = d["output"].get("constraint",
                                                                    opt_constraint)
                elif c["input"]["rem"]["job_type"] == "freq" or c["input"]["rem"]["job_type"] == "frequency":
                    freq = c["frequencies"]
                    enthalpy = c["enthalpy"]
                    entropy = c["entropy"]
                    d["output"]["frequencies"] = d["output"].get("frequencies", freq)
